     ...
     for i= 1:length(box) 
            for j= 1:length(Kernel)   
                        ...
                        if (Kernel(j) == 'linear') 
                                ...
                                %varius transformations required to compute the
                                %confusion matrix, after we calculate
                                %accuracy and Fscore.
                                %Since we have to do that
                                %over different  

                                [Accuracy,AccuracyTrain,Fscore,FscoreTrain] = SVM_calculations(train,val_idx,train_idx,models,counter,z,i,j);
